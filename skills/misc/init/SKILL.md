---
name: init
description: Initialize project idea/todo tracking structure. Use when the user wants Codex to set up the docs/ inbox, todos, areas registry, and ledgers for task tracking.
---

# Init

Use this skill to initialize project idea/todo tracking structure.

Read and follow:

- `playbooks/skills/misc/init.md`
- `playbooks/conventions/todo-convention.md`
- `playbooks/conventions/inbox-convention.md`

Keep this skill thin. The playbook is the shared workflow and should be updated first when the process changes.
