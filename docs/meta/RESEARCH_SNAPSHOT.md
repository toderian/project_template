...
- `plugins/agents-core/seed/AGENTS.md` defaults to sequential role emulation first
...
- added `plugins/agents-core/skills/git-discipline/references/autonomy-levels.md`
...
- added `plugins/agents-core/skills/subagent-protocol/references/prompt-orchestration.md`
- seeded a prompt-orchestration long-task workbook (dropped in 1.0.0)
...
- `plugins/agents-core/seed/AGENTS.md` now explicitly requires surfacing material assumptions and ambiguity
...
| --- | --- | --- | --- |
| 2026-08-19 | [`mattpocock/skills`](https://github.com/mattpocock/skills) at `885e2ca4d842d139e9aef4e48d366c63cb1b8013` (v1.2.3, MIT) | Many of this repo's skills are older ports of this source, which has since been restructured: an interview primitive split out of its routers, an explicit user- vs model-invoked discipline, a multi-session decision map, and a written theory of writing for agents. Reviewing it also dated our own ports | Adapted six skills (`grilling`, `domain-modeling`, `wayfinder`, `research`, `wait-what`, `to-questionnaire`, `writing-for-agents`) into the plugins, rewrote `grill-me`/`grill-with-docs` as routers, slimmed `write-a-skill`, bound wayfinder to the task ledger instead of an issue tracker, and recorded upstream provenance on 21 existing ports. A second pass over the older ports followed: added `codebase-design` as the shared deep-module vocabulary, fixed two defects it surfaced in our own skills (`diagnose` referenced a minimised repro no phase produced and had no redaction rule; `github-triage` wrote already-implemented features into the rejection store it reads back), added the wide-refactor expand-migrate-contract exception, and repointed `ubiquitous-language` at `docs/resources/CONTEXT.md`. Skipped `setup-matt-pocock-skills` (covered by `at init`/`setup-project`), `ask-matt`, `teach`, `git-guardrails` (covered by hooks), the TypeScript-specific skills, and everything under `in-progress/`. Did not vendor any upstream file |
| 2026-07-01 | [`multica-ai/andrej-karpathy-skills`](https://github.com/multica-ai/andrej-karpathy-skills) at `2c606141936f1eeef17fa3043a72095b4765b9c2`, plus local subagent audit and plan critique | The maintainer wanted the principles as base repo behavior. The source distilled common coding-agent failure modes but overlapped heavily with existing doctrine and was not license/release-clean enough for verbatim copying | Adapted the portable principles into `plugins/agents-core/seed/AGENTS.md` and `README.md` as project-owned coding discipline; did not vendor the upstream files or add a person-branded always-on skill |
