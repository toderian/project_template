...
metadata:
  source:
    ...
    - "github.com/mattpocock/skills@885e2ca skills/engineering/triage/SKILL.md (already-implemented split, redundancy check, AI disclaimer; adapted)"
...
Every comment or issue this skill posts to the tracker **must** start with the disclaimer line:

```
> *This was generated by AI during triage.*
```

Reporters are entitled to know a machine wrote the reply they are answering.
...
| `unlabeled`    | `wontfix`         | Maintainer (via skill) | Issue is spam, duplicate, or out of scope. Skill closes with comment (and writes `.out-of-scope/` for *rejected* enhancements only). |
...
| `needs-triage` | `wontfix`         | Maintainer (via skill) | Maintainer decides not to action. Skill closes with comment (and writes `.out-of-scope/` for *rejected* enhancements only).          |
...
- **Redundancy check:** search for an existing implementation of the requested behavior **by domain
  concept, not by the request's wording**, and report where you looked. If the behavior already
  exists, this is an already-implemented `wontfix` (Step 5)
- **Prior-rejection check:** read `.out-of-scope/*.md` files and check if this issue matches or is
  similar to a previously rejected concept
...
- A brief summary of what you found in the codebase that's relevant, including whether the behavior
  already exists
...
If the issue needs to be fleshed out before it's ready for an agent, interview the maintainer to build a complete specification. Run the `agents-core:grilling` skill, with two triage-specific rules on top:
...
- **wontfix (already implemented)** — the behavior already exists. Point at where it lives, then close.
  Do **not** write to `.out-of-scope/`: that store is read back as prior *rejections*, so recording a
  built feature there would make future triage reject a request the project actually supports
- **wontfix (enhancement, rejected)** — write to `.out-of-scope/`, post a comment linking to it, then close the issue (see [OUT-OF-SCOPE.md](references/OUT-OF-SCOPE.md))
