...
check_path() {
  ...
  case "${FILE_PATH}" in
    */docs/tasks_manager/_todos/*|docs/tasks_manager/_todos/*)
      ...
      CONVENTION="the agents-tasks:task-ledger skill (references/todo-convention.md)" ;;
    */docs/tasks_manager/_inbox/*|docs/tasks_manager/_inbox/*)
      LAYER="_inbox"; ARCHIVE="_inbox_archived"
      TERMINAL_RE='^(promoted|dropped)$'
      CONVENTION="the agents-tasks:task-ledger skill (references/inbox-convention.md)" ;;
    *) return 0 ;;
  esac
