...
This is the heavyweight form of `agents-core:planning-workflow`. Choose it when parallel subagent dispatch is already on the table.
...
- A local PRD file.
- An existing `docs/_plans/<feature>.md` produced by `agents-core:planning-workflow`.
- A rough intent the user types directly.

This skill does **not** replace the PRD chain (`agents-extras:write-a-prd`, `agents-core:planning-workflow` from a PRD, `agents-extras:prd-to-issues`, `agents-tasks:prd-to-todos`). `spec.md` is per-work-item engineering intent, not a product PRD. If a PRD already exists, summarize it into `spec.md`; if not, start from intent.
