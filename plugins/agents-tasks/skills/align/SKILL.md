---
name: align
description: "Check a proposed feature or change against PROJECT.md (vision, goals, scope, constraints) and report ALIGNED, NEEDS_CLARIFICATION, or OUT_OF_SCOPE. Use when the user wants to validate alignment before starting non-trivial work or before planning. Requires PROJECT.md at the repo root."
metadata:
...
> `PROJECT.md` is required for `/align`. Copy the PROJECT.md template into place and fill in at least Vision, Goals, and Out of scope. The other sections (Constraints, Current phase, Known limitations) sharpen the verdict but are not required.
