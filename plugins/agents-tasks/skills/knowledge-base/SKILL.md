...
metadata:
  source:
    [
      playbooks/conventions/knowledge-base-quickstart.md,
      playbooks/conventions/runbook-convention.md,
      playbooks/conventions/adr-convention.md,
      playbooks/conventions/generated-artifacts.md,
      playbooks/templates/adr.template.md,
      playbooks/templates/runbook.template.md,
      playbooks/templates/runbook.local.template.md,
      playbooks/templates/area-sources.template.md,
      playbooks/templates/resource-inbox-batch.template.md,
    ]
paths: ["docs/resources/**", "docs/areas/**"]
---
...
This skill defines where durable project knowledge lives in template-inherited repos. The goal is
that agents can refresh context without guessing whether a root file, task page, or co-located note is
authoritative.

- `references/runbook-convention.md` — the exact runbook format, placeholders, and local-bindings
  split; starter templates are `assets/runbook.template.md` and `assets/runbook.local.template.md`.
- `references/adr-convention.md` — the three-part ADR offer test, format, and when to skip one; starter
  template is `assets/adr.template.md`.
- `references/generated-artifacts.md` — stable-artifact vs. rerunnable-report naming, timestamp format,
  and delta sections.
- `assets/area-sources.template.md` — starter for an area's `sources.md` source-history table.
- `assets/resource-inbox-batch.template.md` — starter `README.md` manifest for a raw-knowledge inbox
  batch folder.
...
- `docs/areas/<area>.md` remains the generated task-status page produced by `at ledger sync`. It may
  include a generated pointer to the durable area context, but agents should not add durable
...
manifest using `assets/resource-inbox-batch.template.md`, plus committed Markdown files such as
`transcript.md`, `notes.md`, or `chat-export.md` when available. Raw audio, video, archives, and other
large or sensitive files stay ignored or external by default unless the downstream project
...
`assets/area-sources.template.md` for the initial file.
...
Use `references/runbook-convention.md` (in this skill) for the exact format and
`assets/runbook.template.md` for a starter.

## Architecture decisions

Use `references/adr-convention.md` (in this skill) for the three-part offer test (hard to reverse,
surprising without context, the result of a real trade-off), the location/numbering rule
(`docs/adr/NNNN-slug.md`), and the format. Use `assets/adr.template.md` for a copyable starter.

## Generated artifacts

Use `references/generated-artifacts.md` (in this skill) to decide whether a file is a stable artifact
(keeps a stable ID/slug/path, no timestamp) or a rerunnable report (gets a new timestamped filename per
run under `docs/resources/_reports/<workflow>/`, with a delta section when a prior report exists).
