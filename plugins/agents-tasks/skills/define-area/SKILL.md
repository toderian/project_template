---
name: define-area
description: "Define or refresh a durable docs/resources/<area>/ context for a domain or product capability, including participant repos, dependency graph, install modes, runtime dependencies, and known docs. Use when the user says \"define area\", \"index area\", \"map area\", or asks to establish cross-repo area knowledge."
metadata:
  source: playbooks/skills/engineering/define-area.md
  pack: docs-knowledge
---
...
- Use repo slugs from `.config/repos.project.md` when that registry exists. If it is missing, propose
  running `at init --with-repos` to create it before committing cross-repo docs that need stable repo names.
...
confirmation. Pick a unique uppercase prefix and let `at ledger` create or refresh
