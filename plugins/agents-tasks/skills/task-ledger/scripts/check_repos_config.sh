...
usage() {
  cat <<'EOF'
Usage: at repos-check [--local] [--root DIR]
...
Options:
  ...
  --root DIR  Repository root (default: git toplevel of the current directory, else the
              current directory).
...
command -v python3 >/dev/null 2>&1 || {
  echo "error: required command 'python3' not found on PATH (used to validate .config/repos.project.md and .local/repos.map)" >&2
  exit 2
}

python3 - "${REPO_ROOT}" "${LOCAL_MODE}" "${SCRIPT_DIR}" <<'PY'
...
# Share the escape-aware table-row splitter with sync_todo_ledgers.py (both live
# in task-ledger/scripts/) so the two tools agree on how a Markdown row
# (including escaped `\|`) parses.
    ...
    if not path.exists():
        if LOCAL_MODE:
            report(
                path,
                None,
                "missing repo registry; run `at init --with-repos` to create .config/repos.project.md",
            )
        return {}, False
...
def load_local_map(registry: dict[str, dict[str, str]]) -> dict[str, str]:
    ...
    if not path.exists():
        report(path, None, "missing local repo map; create .local/repos.map by hand (see the task-ledger skill, references/todo-convention.md)")
        return mappings
