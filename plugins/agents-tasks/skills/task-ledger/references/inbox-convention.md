...
[todo-convention.md](todo-convention.md) for the task layer.
...
```bash
at reserve inbox <short-description>
```
...
1. Reserve the file with `at reserve inbox <short-desc>`.
...
- **Promote** — it's worth doing. Create a full task per `todo-convention.md`: assign the next
  ...
  acceptance criteria and related tests, and run `at ledger sync`. Then set the inbox
