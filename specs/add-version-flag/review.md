# Add `--version` flag to `_base/scripts/gen-skills-table.sh` — Review log

## Iteration 1 — 2026-05-12

**Dispatched against:** unstaged diff against HEAD (`bdddc5a`) on `_base/scripts/gen-skills-table.sh`.
...
- **SC1** — `./_base/scripts/gen-skills-table.sh --version` produced exactly `gen-skills-table.sh 0.1.0\n` (`xxd` confirms 25 bytes ending in `0a`); exit 0. **PASS**
