...
- `/init` — ensures the canonical `docs/tasks_manager/` + `docs/reference/` layout is present.
...
|------|--------------------|-------------|-----|
| **Loose work** | ad-hoc `TODO.md`, `NOTES.md`, inline `TODO:`/`FIXME:` clusters, half-finished todo files not matching `T-NNN` | `docs/tasks_manager/_inbox/` as `I-NNN` ideas | The inbox is the cheap, reversible capture layer. Re-triaging later beats importing low-quality todos as first-class backlog. |
| **Loose docs** | design notes, stray READMEs, architecture scribbles, `*.md` outside `docs/` that explain *how the system works* | `docs/reference/` | That's the home `/init` seeds for project documentation. |
...
Confirm `docs/tasks_manager/` and `docs/reference/` exist. If not, run `/init` (or its playbook
...
Write a migration report to `docs/reference/_tidy-report.md` (and summarize it in chat). The report is
...
## Summary
N loose-work items → inbox · M loose docs → reference · K orphans flagged
...
## Loose docs → docs/reference/ (proposed move)
| Source | Proposed destination |
|--------|---------------------|
| notes/auth-design.md | docs/reference/auth-design.md |
...
used. Best-guess `Area` from `docs/tasks_manager/_areas.md`; `—` is fine.
...
- **Loose docs → reference.** `git mv` each into `docs/reference/` (use `git mv` to preserve history).
...
natural next step: **`/triage-inbox`** to promote the freshly-swept ideas into typed `T-NNN` todos. The
sweep deliberately leaves them as raw `I-NNN` — tidy gets the mess into the right buckets; triage
...
- The report (`docs/reference/_tidy-report.md`) is self-contained — a reader can see what was found,
