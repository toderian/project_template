...
Adds `academic-humanizer` as a new shared playbook skill under `playbooks/skills/personal/`, adapted
from the MIT-licensed `AIScientists-Dev/academic-humanizer` skill with attribution. The skill covers
academic manuscript, thesis, rebuttal, abstract, NSF/NIH Specific Aims, Project Summary, and
grant-proposal prose editing while preserving numbers, citations, claims, evidence strength, and
AI-use disclosure integrity.

- `.agents/skill-library.json` now lists `academic-humanizer` in the `writing` pack and updates that
  pack description to mention academic humanizing.
- The default `recommended` selection is unchanged (`core`, `security`, `task-management`, `ui`), so
  no new runtime wrapper, plugin manifest entry, or active skill is generated by default.
- The playbook intentionally complements the existing `deslop` and `sciwrite` skills instead of
  vendoring the upstream file verbatim.

**Downstream impact:** Repos using the default `recommended` profile are unaffected; the skill remains
inactive because `writing` is not part of that profile. Repos that enable `writing`, use `full`, or
explicitly enable `academic-humanizer` will expose the new skill after running the normal skill
synchronization. No generated wrappers are present until the skill is active.
...
The 6 completed plans documenting the **template's own construction** (`autonomy-aware-loop-engineering.md`,
`central-artifact-registry.md`, `dated-roadmap-milestones.md`, `human-reusable-workflow-artifacts.md`,
`safe-workspace-commit-squash-skill.md`, `task-native-specs-system-map.md`) move from `docs/_plans/` to
`playbooks/meta/template-plans/` (new `README.md` marks them historical). `docs/_plans/` now holds only
`.gitkeep`, matching the seed skeleton (`_base/docs/_plans/.gitkeep`), so a freshly seeded downstream
project's durable-plans directory starts empty instead of pre-populated with the template's own
construction history mislabeled as the project's plans. The `docs/_plans/` **lane** itself is unchanged:
it remains the durable-plans location for a downstream project's own work, exactly as described in
`_base/AGENTS.md` and `playbooks/conventions/generated-artifacts.md`.
