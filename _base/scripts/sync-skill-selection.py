...
README_BEGIN_MARKER = "<!-- BEGIN skills-table -->"
README_END_MARKER = "<!-- END skills-table -->"
...
def readme_path(root: Path) -> Path:
    return root / "_base" / "README.md"


def readme_table_block(active: list[Skill]) -> str:
    lines = [
        README_BEGIN_MARKER,
        "<!-- This block is auto-generated by _base/scripts/sync-skill-selection.py"
        " from playbook frontmatter — do not edit by hand. -->",
        "",
        "| Skill | Bucket | Description |",
        "|-------|--------|-------------|",
    ]
    for skill in sorted(active, key=lambda s: (s.name, s.bucket)):
        desc = skill.description.replace("|", "\\|")
        lines.append(f"| {skill.name} | {skill.bucket} | {desc} |")
    lines.append("")
    lines.append(README_END_MARKER)
    return "\n".join(lines)


def splice_readme_table(root: Path, active: list[Skill]) -> tuple[str, str]:
    """Return (current, updated) README text with the skills-table block refreshed."""
    path = readme_path(root)
    current = path.read_text()
    if README_BEGIN_MARKER not in current or README_END_MARKER not in current:
        raise SystemExit(f"{path.relative_to(root)} is missing the skills-table markers")
    start = current.index(README_BEGIN_MARKER)
    stop = current.index(README_END_MARKER) + len(README_END_MARKER)
    updated = current[:start] + readme_table_block(active) + current[stop:]
...
def check_sync(root: Path, active: list[Skill], skills: dict[str, Skill]) -> list[str]:
    ...
    current_readme, updated_readme = splice_readme_table(root, active)
    if current_readme != updated_readme:
        findings.append("drift _base/README.md (skills-table)")
...
def write_sync(root: Path, active: list[Skill], skills: dict[str, Skill]) -> None:
    ...
    current_readme, updated_readme = splice_readme_table(root, active)
    if current_readme != updated_readme:
        readme_path(root).write_text(updated_readme)
