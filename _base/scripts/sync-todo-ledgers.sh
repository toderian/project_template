...
usage() {
  cat <<'EOF'
Usage: _base/scripts/sync-todo-ledgers.sh [--check]
...
declare -A AREA_OPEN AREA_INPROGRESS AREA_DONE AREA_URGENT AREA_NOW AREA_NEXT AREA_LATER AREA_SOMEDAY
declare -a TASK_IDS=()
...
write_or_check() {
  ...
  if [[ "${CHECK_MODE}" -eq 1 ]]; then
    ...
    if ! cmp -s "$source" "$target"; then
      error "${label} is stale at $(rel_repo "$target"); run _base/scripts/sync-todo-ledgers.sh"
    fi
  else
    cp "$source" "$target"
  fi
...
`_base/scripts/sync-todo-ledgers.sh`. The task files in `docs/tasks_manager/_todos/` remain the source of truth.
  ...
  while IFS= read -r line; do
    ...
    if [[ "$horizon" == "__invalid__" ]]; then
      while IFS= read -r id; do
        [[ -n "$id" ]] || continue
        validate_or_warn "roadmap reference '${id}' appears outside an Urgent/Now/Next/Later/Someday horizon"
...
for id in "${ROADMAP_INBOX_IDS[@]}"; do
  ...
  if [[ "${ROADMAP_INBOX_HORIZON[$id]:-}" != "Someday" ]]; then
    validate_or_warn "roadmap inbox reference '${id}' appears in ${ROADMAP_INBOX_HORIZON[$id]:-unknown} but inbox ideas may only appear in Someday; promote with /triage-inbox before scheduling into Urgent, Now, Next, or Later"
  fi
...
ensure_area_page() {
  ...
  title="${area//-/ }"
  {
    printf '# %s\n\n' "$title"
    printf '%s\n\n' "${AREA_DESC[$area]}"
    printf 'Area context: [../resources/%s/summary.md](../resources/%s/summary.md).\n\n' "$area" "$area"
    printf '<!-- BEGIN generated-area-status -->\n'
    printf '_Generated by `_base/scripts/sync-todo-ledgers.sh`. Do not edit this block by hand._\n\n'
    printf '## Urgent\n\n_No tasks._\n\n'
    printf '## Now\n\n_No tasks._\n\n'
    printf '## Next\n\n_No tasks._\n\n'
    printf '## Later\n\n_No tasks._\n\n'
...
for area in "${AREAS[@]}"; do
  ...
  block="$(new_tmp)"
  {
    printf '_Generated by `_base/scripts/sync-todo-ledgers.sh`. Do not edit this block by hand._\n\n'
    printf '## Urgent\n\n'
    section_for_area_horizon "$area" "Urgent"
    printf '\n## Now\n\n'
    ...
    printf '\n## Someday\n\n'
    section_for_area_horizon "$area" "Someday"
    printf '\n## Unscheduled\n\n'
...
{
  printf '_Generated by `_base/scripts/sync-todo-ledgers.sh`. Do not edit this block by hand._\n\n'
  printf '| Area | Prefix | Description | Urgent | Now | Next | Later | Someday | Open | In Progress | Done | Page |\n'
  printf '|------|--------|-------------|--------|-----|------|-------|---------|------|-------------|------|------|\n'
  for area in "${AREAS[@]}"; do
    page_abs="$(page_abs_for "${AREA_PAGE[$area]}")"
