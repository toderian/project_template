...
agy --version
agy --help
./_base/scripts/sync-skill-selection.py --check
./_base/scripts/check-antigravity-skills.sh
```

The Antigravity wrappers are generated by `./_base/scripts/sync-skill-selection.py --sync` (which also
writes the Codex/Claude wrappers, the manifest, and the README table). `gen-antigravity-skills.sh` is a
deprecated shim that forwards to it and is removed next release.
