...
└── .agents/
    ├── skill-library.json                 # Skill selection: packs, profiles, agent roles
...
.agents/skill-library.json               →  selection only: packs, profiles, and agent roles
...
`.claude-plugin/plugin.json`, the runtime wrapper trees, and the skills table in this README. Active runtime
wrappers are generated surfaces, not the durable source of truth.
...
<!-- BEGIN skills-table -->
<!-- This block is auto-generated by _base/scripts/sync-skill-selection.py from playbook frontmatter — do not edit by hand. -->
...
2. Create the playbook `playbooks/skills/<bucket>/<name>.md`, opening with a `---` frontmatter block that
   sets `name`, `description` (JSON-encoded string), and optional `argument-hint`.
3. Add the skill's `name` to one or more packs in `.agents/skill-library.json`.
4. If it should be active in this repo, add its pack or name to `.agents/skills.enabled.json`, or run
   `./_base/scripts/setup-agents.sh --skills ...`.
5. Regenerate manifests, wrappers, and the skills table: `./_base/scripts/sync-skill-selection.py --sync`.
6. Validate consistency: `./_base/scripts/check-skills-sync.sh` and
   `./_base/scripts/check-antigravity-skills.sh` (fix any findings and re-run until clean).
7. For Codex, run `skills/install-codex-skills.sh` and restart Codex.
...
wrappers are absent, every bucket-level playbook carries metadata frontmatter, the auto-generated skills
table is up-to-date, and personalities are not exposed as slash commands. It calls
`sync-skill-selection.py --check` (which now also validates the README skills table), so validation is
read-only. Designed to be called by an agent in a loop:
...
```bash
./_base/scripts/sync-skill-selection.py --sync   # regenerates all wrappers, manifest, and README table
...
| `.agents/skill-library.json` and `.agents/skills.enabled.json` | Both | Skill selection (packs/profiles/agent roles) and active profile/packs |
