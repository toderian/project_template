...
| cross-repo-pr-review | engineering | Review multiple linked pull requests as one cross-repo feature, validating contracts, rollout order, compatibility, CI, and per-PR risks. Use when the user says "cross-repo PR review", "review these PRs together", "multi-repo PRs", or asks to validate PRs across core libraries, backends, UIs, services, or clients. |
...
| describe-component | engineering | Generate or refresh a docs-primary component CONTEXT.md covering responsibility, public interface, dependencies, owned data, invariants, and tests. Use when the user wants to "describe a component", "document this module/service", map subsystem boundaries, or onboard/handoff code. |
...
| doubt-driven-development | engineering | Adversarial review for a non-trivial implementation decision before it stands. Use when the user asks to "doubt" or stress-test a decision, when correctness matters more than speed, or when the choice crosses module boundaries, asserts something hard to verify, or is hard to reverse. |
...
| map-system | engineering | Create or refresh docs/resources/system-map.md as the status-aware repo, capability, flow, and boundary index. Use when the user asks to map the system, index working repos, refresh repo/capability architecture, or understand how repos fit together. |
...
| migration-safety | engineering | Safety review and generation guidance for reversible database schema migrations. Focuses on production hazards such as table-rewrite locks, unsafe NOT NULL changes, missing concurrent indexes, irreversible drops, backfills, and zero-downtime DDL across PostgreSQL, MySQL, SQLite, Liquibase, or Flyway. |
