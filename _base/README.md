...
| prd-to-todos | productivity | Extract actionable todos from a PRD and create todo files. Use when user wants to convert a PRD into trackable todo items in docs/tasks_manager/_todos/. |
...
| roadmap | productivity | Maintain docs/tasks_manager/_roadmap.md — the Now/Next/Later plan of execution across all todos and ideas, with collapsible per-task subtasks. Use when the user wants to "plan the roadmap", "what's next", reprioritize/sequence work, schedule todos, or refresh the roadmap. Distinct from per-change planning (planning-workflow / prd-to-plan). |
